...
export const MECHANISMS: Mechanism[] = [
  ...
  // ────────────────────────────────────────────────────────────
  {
    ...
    subtitle:
      "CDN에서 받는 엔진 셋(qpdf·libheif·libavif)이 이 문을 지난다. 해시가 어긋나면 기능을 포기할지언정 실행하지 않는다.",
    kind: "flow",
    features: ["common-wasmloader", "pdf-password", "pdf-recompress", "image-decode", "image-pipeline"],
      ...
      nodes: [
        {
          id: "need",
          label: "엔진이 필요해진 순간",
          note: "PDF의 암호·다시 압축, HEIC 열기, AVIF 굽기에서만. 그전에는 아무것도 받지 않는다",
          kind: "input",
          col: 0,
          row: 1,
        },
  ...
  {
    id: "video-concat",
    title: "구간 여러 개를 한 파일로 잇기",
    subtitle:
      "mediabunny 1.52.3에는 여러 입력 구간을 한 출력에 먹이는 API가 없다. Conversion은 입력 하나만 맡고 trim도 한 쌍뿐이라, 출력에 패킷 소스를 직접 달고 옮긴다.",
    kind: "flow",
    features: ["video-concat", "video-probe"],
    techs: ["mediabunny", "webcodecs"],
    src: [
      "apps/video/src/lib/video/segments.ts",
      "apps/video/src/lib/video/concat.ts",
      "apps/video/src/lib/video/probe.ts",
    ],
    flow: {
      nodes: [
        {
          id: "list",
          label: "구간 목록",
          note: "목록 순서가 곧 결과 순서다. 겹침·순서 뒤바뀜은 고치지 않고 배지로만 알린다",
          kind: "input",
          col: 0,
          row: 1,
        },
        {
          id: "gate",
          label: "패킷 복사로 될까",
          note: "컨테이너가 코덱을 담는가 · 회전이 복사를 깨는가 · 구간마다 시작이 키프레임인가. 코덱 파라미터는 안 본다 — 전부 같은 파일의 같은 트랙이다",
          kind: "gate",
          col: 1,
          row: 1,
        },
        {
          id: "copy",
          label: "EncodedPacketSink → clone(시각 + 오프셋) → PacketSource",
          note: "시작은 그 자리의 키프레임으로 내려간다. 오디오는 구간 시작을 걸친 패킷을 버려서 앞 20ms 안팎이 빠진다",
          col: 2,
          row: 0,
        },
        {
          id: "recode",
          label: "구간마다 Conversion으로 다시 굽기",
          note: "시작을 조금이라도 자르면 엔진의 복사 경로가 꺼진다(needsTranscode가 firstTimestamp < startTimestamp를 본다) — 구간 잘라 잇기는 언제나 그 경우다",
          col: 2,
          row: 2,
        },
        {
          id: "check",
          label: "조각의 디코더 설정이 같은가",
          note: "코덱 이름과 description 바이트를 견준다. 다르면 거부 — 그대로 이으면 첫 조각의 설정으로 뒤 조각이 깨져 나간다",
          kind: "gate",
          col: 3,
          row: 2,
        },
        {
          id: "join",
          label: "조각을 패킷 단위로 잇기",
          note: "각 조각은 0에서 시작하는 완결된 파일이라 앞 조각이 끝난 자리만큼 시각을 민다",
          col: 4,
          row: 2,
        },
        { id: "out", label: "MP4 · WebM 한 개", kind: "output", col: 5, row: 1 },
        {
          id: "stop",
          label: "이어붙이기 거부",
          note: "구간마다 코덱 설정이 다르다고 화면에 적는다",
          kind: "reject",
          col: 4,
          row: 3,
        },
      ...
      edges: [
        { from: "list", to: "gate" },
        { from: "gate", to: "copy", label: "된다", kind: "ok" },
        { from: "gate", to: "recode", label: "안 된다" },
        { from: "recode", to: "check" },
        { from: "check", to: "join", label: "같다", kind: "ok" },
        { from: "check", to: "stop", label: "다르다", kind: "fail" },
        { from: "copy", to: "out" },
        { from: "join", to: "out" },
      ],
    },
  ...
  {
    id: "pdf-shrink",
    title: "PDF 용량을 줄이는 두 갈래",
    subtitle:
      "글자를 지키면 덜 줄고, 많이 줄이면 글자를 잃는다. 되돌릴 수 없는 쪽을 자동으로 고르지 않는 이유다.",
    kind: "flow",
    features: ["pdf-recompress", "pdf-raster"],
    techs: ["qpdf", "pdfjs", "pdflib", "sizesearch"],
    src: [
      "apps/pdf/src/lib/pdf/qpdfLoader.ts",
      "apps/pdf/src/lib/pdf/repack.ts",
      "apps/pdf/src/lib/pdf/compress.ts",
    ],
    flow: {
      nodes: [
        {
          id: "in",
          label: "PDF 한 개",
          note: "쪽 수와 글자 유무를 먼저 잰다. 여는 순서를 문서 전체에 흩고 1.5초에서 끊는다 — 훑다 만 문서에 '글자 없음'을 붙이면 되돌릴 수 없는 쪽을 안심하고 누르게 된다",
          kind: "input",
          col: 0,
          row: 1,
        },
        { id: "way", label: "어느 길", kind: "gate", col: 1, row: 1 },
        {
          id: "qpdf",
          label: "--object-streams=generate --compression-level=9 --recompress-flate",
          note: "글자·글꼴·주석은 그대로. 그림까지 줄이려면 --optimize-images --jpeg-quality=75(강하게는 40)가 붙는다. 엔진을 받아야 해서 이 갈래만 인터넷이 필요하다",
          col: 2,
          row: 0,
        },
        {
          id: "draw",
          label: "쪽을 JPEG로 그려 새 PDF에 담기",
          note: "쪽 크기는 72/dpi로 포인트로 되돌린다. 이 환산을 빼면 200dpi 결과가 A4의 2.8배가 된다. 글자는 여기서 사라진다",
          col: 2,
          row: 2,
        },
        {
          id: "ladder",
          label: "목표 용량이면 사다리를 짚는다",
          note: "기본 12칸을 고른 설정 아래로 눌러 세워 5~12칸이 되고, 맨 위 칸은 언제나 그 설정이다. 한 번이 문서 전체 렌더라 횟수를 쪽 수로 깎는다 — 8쪽 이하 여섯 번, 120쪽을 넘으면 두 번",
          col: 3,
          row: 2,
        },
        {
          id: "cmp",
          label: "원본보다 작은가",
          note: "두 갈래가 함께 지나는 판정",
          kind: "gate",
          col: 4,
          row: 1,
        },
        { id: "out", label: "더 작은 PDF", kind: "output", col: 5, row: 0 },
        {
          id: "keep",
          label: "원본을 그대로 내려준다",
          note: "이미 압축된 PDF는 객체 스트림 머리글 때문에 커진다(실측 2394 → 2437바이트). 화면은 '줄지 않았다'고 적는다",
          col: 5,
          row: 2,
        },
      ],
      edges: [
        { from: "in", to: "way" },
        { from: "way", to: "qpdf", label: "다시 압축", kind: "ok" },
        { from: "way", to: "draw", label: "이미지로", kind: "ok" },
        { from: "draw", to: "ladder" },
        { from: "qpdf", to: "cmp" },
        { from: "ladder", to: "cmp" },
        { from: "cmp", to: "out", label: "작다", kind: "ok" },
        { from: "cmp", to: "keep", label: "같거나 크다" },
      ],
    },
  },
  ...
  {
    id: "gif-diff",
    title: "안 바뀐 픽셀은 다시 싣지 않기",
    subtitle:
      "앞 프레임과 같은 자리를 투명 인덱스로 두면 LZW가 짧게 만든다. 팔레트 한 칸을 내주고 얻는 것이라, 언제나 이득은 아니다.",
    kind: "flow",
    features: ["gif-diff", "gif-encode"],
    techs: ["gifdiff", "gifenc"],
    src: ["apps/gif/src/lib/gif/diff.ts", "apps/gif/src/lib/gif/encode.ts"],
    flow: {
      nodes: [
        {
          id: "in",
          label: "앞 프레임 · 이번 프레임 (RGBA)",
          note: "네 바이트를 다 비교한다 — 알파만 달라도 바뀐 것으로 센다",
          kind: "input",
          col: 0,
          row: 1,
        },
        {
          id: "rect",
          label: "다른 픽셀의 최소 사각형",
          note: "한 픽셀도 다르지 않으면 사각형이 없다 — 그 프레임은 1×1 투명 한 장으로 딜레이만 싣는다",
          col: 1,
          row: 1,
        },
        {
          id: "gate",
          label: "차분을 쓸 수 있나",
          note: "알파가 있으면 못 쓴다(투명 자리를 남기려면 disposal 1인데 알파 프레임은 2다) · 16색 미만이면 안 쓴다 · 바뀐 넓이가 90%를 넘으면 안 쓴다",
          kind: "gate",
          col: 2,
          row: 1,
        },
        {
          id: "full",
          label: "프레임 전체를 팔레트에 매핑",
          note: "예전과 같은 길. 차분을 켜 두었으면 다음 프레임이 얹힐 수 있게 disposal 1만 명시한다",
          col: 3,
          row: 0,
        },
        {
          id: "crop",
          label: "사각형만 quantize · applyPalette",
          note: "1920×1080에서 200×100만 바뀌면 두 함수가 보는 픽셀이 1/104로 준다. 두 함수는 버퍼 전체를 읽으므로 자기 버퍼를 가진 사본을 넘긴다",
          col: 3,
          row: 2,
        },
        {
          id: "compose",
          label: "출력 크기 인덱스 배열 · 사각형 밖은 투명",
          note: "잘라 낸 그림을 그 자리에 앉히지 못한다 — gifenc가 프레임 x·y를 0으로 박아 쓴다. 그래서 크기는 그대로 두고 밖을 투명으로 채운다",
          col: 4,
          row: 2,
        },
        {
          id: "write",
          label: "writeFrame · transparent · dispose 1",
          note: "1은 화면 유지. 2면 비워 둔 자리가 지워져 깜빡인다",
          kind: "output",
          col: 5,
          row: 1,
        },
      ...
      edges: [
        { from: "in", to: "rect" },
        { from: "rect", to: "gate" },
        { from: "gate", to: "full", label: "못 쓴다", kind: "fail" },
        { from: "gate", to: "crop", label: "쓴다", kind: "ok" },
        { from: "crop", to: "compose" },
        { from: "compose", to: "write" },
        { from: "full", to: "write" },
      ],
    },
  },
