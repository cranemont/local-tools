/** PWA 파일 연결 — 설치된 앱이면 .csv/.xlsx 더블클릭이 이 앱으로 들어온다.
 *
 * 이 앱이 존재하는 이유의 절반이 여기 있다: 맥에서 CSV를 더블클릭하면 Numbers가
 * 뜨는 걸 실제로 바꾸는 방법이 브라우저에는 이것 하나뿐이다(Chromium 데스크톱 전용).
